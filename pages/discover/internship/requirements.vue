	...
	<view class="requirements-page">
		<view class="nav-bar" :style="{paddingTop: statusBarHeight + 'px'}">
			<view class="nav-bar__content">
				...
				<text class="nav-bar__title">实习要求</text>
		...
		<scroll-view class="content-scroll" scroll-y @scrolltolower="loadMore"
			...
			<view v-else-if="!loading && records.length === 0" class="state-container">
				<view class="state-icon-wrap">
					<l-icon name="file-document" style="font-size: 48px; color: #94a3b8;"></l-icon>
				</view>
				<text class="state-text">暂无实习要求</text>
			</view>
			...
			<view v-else class="list-container">
				<view v-for="(item, index) in records" :key="index" class="req-card">
					<view class="req-card__header">
						<text class="req-card__title">{{ item.title || item.requirementTitle || '实习要求' }}</text>
					</view>
					<view class="req-card__body">
						<view class="req-card__row" v-if="item.content">
							<text class="req-card__content">{{ item.content }}</text>
						</view>
						<view class="req-card__row" v-if="item.publishDate || item.createTime">
							<l-icon name="calendar" style="font-size: 14px; color: #94a3b8; margin-right: 6rpx;"></l-icon>
							<text class="req-card__info">{{ item.publishDate || item.createTime }}</text>
						</view>
						<view class="req-card__row" v-if="item.academicYear || item.semester">
							<l-icon name="school" style="font-size: 14px; color: #94a3b8; margin-right: 6rpx;"></l-icon>
							<text class="req-card__info">{{ item.academicYear }} {{ item.semester }}</text>
						</view>
					</view>
				</view>
...
<script>
import { getInternshipRequirementsList } from '../../api/discover.js';
...
export default {
	...
	methods: {
		...
		async fetchData(reset = false) {
			...
			this.error = '';
			try {
				const res = await getInternshipRequirementsList({ pageNo: this.pageNo, pageSize: this.pageSize });
				...
				console.error('获取实习要求失败', e);
				this.error = e.message || '获取实习要求失败';
			} finally {
				this.loading = false;
...
.requirements-page { width: 100%; height: 100vh; display: flex; flex-direction: column; background-color: var(--bg-secondary); }
.nav-bar { background: linear-gradient(135deg, #3b82f6, #60a5fa); flex-shrink: 0; }
...
.state-spinner { width: 48rpx; height: 48rpx; border: 4rpx solid rgba(59, 130, 246, 0.15); border-top-color: #3b82f6; border-radius: 50%; animation: spin 0.8s linear infinite; }
...
.state-icon-circle--error { background: rgba(59, 130, 246, 0.1); color: #3b82f6; }
.state-btn { margin-top: 8rpx; padding: 12rpx 40rpx; background: linear-gradient(135deg, #3b82f6, #60a5fa); color: #fff; font-size: 26rpx; font-weight: 600; border-radius: 32rpx; border: none; }
.list-container { padding: 16rpx 24rpx; }
.req-card { background: #fff; border-radius: 24rpx; padding: 28rpx; margin-bottom: 16rpx; box-shadow: 0 2rpx 12rpx rgba(59, 130, 246, 0.06); border: 1px solid rgba(148, 163, 184, 0.1); }
.req-card__header { margin-bottom: 12rpx; }
.req-card__title { font-size: 30rpx; font-weight: 700; color: var(--text-primary); }
.req-card__body { display: flex; flex-direction: column; gap: 8rpx; }
.req-card__row { display: flex; align-items: flex-start; }
.req-card__content { font-size: 24rpx; color: var(--text-secondary); line-height: 1.6; }
.req-card__info { font-size: 24rpx; color: var(--text-secondary); }
.load-more { display: flex; justify-content: center; align-items: center; padding: 32rpx 0; }
.load-more__text { font-size: 26rpx; color: #3b82f6; font-weight: 600; }
</style>
